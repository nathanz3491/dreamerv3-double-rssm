...
- **Decide the `tomain` → `main` question**: rename/fast-forward `main` to the `tomain` tip (or merge it), and decide whether/when to push to `origin/main` (39+ commits ahead now, unpushed, growing).
...
- **Run the actual comparison** the plan calls for: baseline vs. Stage 3 vs. Stage 4 vs. Stage 5 vs. survival-shaping on/off, using `tools/mapwatch.py` to watch RSSM-2 metrics live and `tools/death_eval.py` to check whether survival shaping actually reduces thirst/hunger deaths.
...
- Keep this file updated on every future run of the parent workspace's scheduled task — the 2026-09-13 session almost repeated the same gap this file's earlier "Gap notice" already documented once.
